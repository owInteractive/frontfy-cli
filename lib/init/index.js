...
module.exports = {
			...
			async function exec() {
				...
				fileSystem
					...
					.then(async () => {
						...
						if (technology.toLowerCase() === 'nodejs') await createEnvFile();
			...
			function cloneRepository() {
				...
				return new Promise(async (resolve, reject) => {
					...
					switch (technology.toLowerCase()) {
						case 'nodejs':
							url = "https://github.com/owInteractive/frontfy.git";
							break;
						case 'php':
							url = "https://github.com/owInteractive/frontfy-php.git";
							break;
						default:
								...
								if (data.error) {
									...
									if (opts.repository === 'github') {
										...
										if(data.data.errors.length > 0) {
											message += ' Error message: ' + data.data.errors[0].message;
										}
						...
						return new Promise(async (resolve, reject) => {
							...
							function gitInit() {
								...
								return new Promise((resolve, reject) => {
									...
									cmd.exec('git init', {
										...
										if (error) {
											spinner.fail('Error on running "git init". Do it manually!');
										}
							...
							function gitAddRemoteServer() {
								...
								return new Promise((resolve, reject) => {
									...
									spinner.succeed(`Executing: git remote add origin ${remoteServerUrl}`);
									...
									cmd.exec(`git remote add origin ${remoteServerUrl}`, {
										...
										if (error) {
											...
											spinner.fail(`Error on running "git remote add origin ${remoteServerUrl}". Do it manually!`);
							...
							function gitAddFiles() {
								...
								return new Promise((resolve, reject) => {
									...
									cmd.exec(`git add .`, {
										...
										if (error) {
											spinner.fail('Error on running "git add .". Do it manually!');
										}
							...
							function gitCommit() {
								...
								return new Promise((resolve, reject) => {
									...
									cmd.exec(`git commit -m "Initial commit"`, {
										...
										if (error) {
											spinner.fail('Error on running "git commit". Do it manually!');
										}
							...
							function gitPush() {
								...
								return new Promise((resolve, reject) => {
									...
									cmd.exec(`git push -u origin master`, {
										...
										if (error) {
											spinner.fail('Error on running "git push -u origin master". Do it manually!');
										}
	...
	run: (name) => {
		...
		return new Promise(async (resolve, reject) => {
			...
			spinner.info('Info: Enter "npm run build" in the project folder to run the project');
			spinner.info('Info: Press "command + c" in the project folder to stop the project');
			spinner.info('Info: Remember to configurate the .env file in /config');
			spinner.info('Info: See the Frontfy CLI documentation on https://github.com/owInteractive/frontfy-cli');
			spinner.info('Info: See the Frontfy documentation on https://github.com/owInteractive/frontfy');
			...
			spawn.stdout.on('data', (data) => {
				...
				if (data.toString().match('Ready!')) {
					...
					runningServerTimer = setTimeout(() => {
						...
						spinner.succeed('Executing: npm run build');
